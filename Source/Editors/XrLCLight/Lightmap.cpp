...
void CLightmap::Capture		(CDeflector *D, int b_u, int b_v, int s_u, int s_v, BOOL bRotated)
{
	...
	if (lm.surface.empty())	lm.create(GetLmapSize(), GetLmapSize());
	
	// Addressing
	xr_vector<UVtri>	tris;
	D->RemapUV			(tris,b_u+BORDER,b_v+BORDER,s_u-2*BORDER,s_v-2*BORDER,GetLmapSize(),GetLmapSize(),bRotated);
	...
	if (!bRotated) 
	{
		...
		blit	(lm,GetLmapSize(),GetLmapSize(),L,real_W,real_H,b_u,b_v,254-BORDER);
	} else {
		u32 real_H	= (L.height	+ 2*BORDER);
		u32 real_W	= (L.width	+ 2*BORDER);
		blit_r	(lm,GetLmapSize(),GetLmapSize(),L,real_W,real_H,b_u,b_v,254-BORDER);
	}
}
...
void CLightmap::Save( LPCSTR path )
{
	...
	// Borders correction
	Status			("Borders...");
	for (u32 _y=0; _y<GetLmapSize(); _y++)
	{
		for (u32 _x=0; _x<GetLmapSize(); _x++)
		{
			u32	offset	= _y*GetLmapSize()+_x;
