...
void CBuild::xrPhase_MergeGeometry()
{
	...
	if (use_fast_way)
	{
		...
		{
			if (mat.second.faces_vec.size() < 24000)
		...
		{
			int MAX_THREADS = lc_global_data()->getMaxThreads();
		...
		{
			int MAX_BIG = reserved_big_objects.size();
			if (MAX_BIG > lc_global_data()->getMaxThreads() - 2)
			{
				MAX_BIG = lc_global_data()->getMaxThreads() - 2;
			}


			std::thread* th = new std::thread[MAX_BIG];
			for (auto i = 0; i < MAX_BIG; i++)
				th[i] = std::thread([&]()
					{
						u32 last_ms = 0;
						for (;;)
