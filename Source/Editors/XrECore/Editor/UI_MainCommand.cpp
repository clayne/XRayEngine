...
/*
    if (bAllowLogCommands)
    {
        ...
        if (p1.IsString() && p2.IsString())
            Msg("%s%s (\"%s\", \"%s\")", level, CMD->Name(), sp1.c_str(), sp2.c_str());
        else
            if (p1.IsInteger() && p2.IsInteger())
                Msg("%s%s (%d, %d)", level, CMD->Name(), u32(p1), u32(p2));
        else
            if (p1.IsInteger() && p2.IsString())
                Msg("%s%s (%d, \"%s\")", level, CMD->Name(), u32(p1), sp2.c_str());
        else
            if (p1.IsString() && p2.IsInteger())
                Msg("%s%s (\"%s\", %d)", level, CMD->Name(), sp1.c_str(), u32(p2));
    }
*/
    exec_level++;
